...
The tmux smoke script also submits from an actual PTY with a freshly started daemon, closes the submitting terminal, and destroys only its own tmux server while the task is pending. Both log streams, the final exit code and the wake-up hook must still complete. The Rust remote-wrapper test exercises launcher exit and SIGHUP before task completion. These checks run through the existing CI gates; they do not replace testing the intended server's logout/resource policies.
...
Before pushing, check both file contents and commit author/committer metadata for personal information. Use your GitHub noreply email for local and web-based commits. Never put private account names, home directories, server addresses, session IDs or raw diagnostic dumps into release evidence. Secret scanners do not reliably detect personal information; review it separately, including historical versions and release attachments.

...
Git SSH read/write access has now been verified using the repository's designated key and a repository-local HostName override for the owner's intentional fake-IP proxy setup. The candidate is pushed to `release/preflight`; that branch matches the CI push trigger. System DNS, routes, TUN and proxy settings were not changed. The current GitHub CLI account is `Erican-Ji`, and its repository API lookup returned 404; SSH key access and GitHub API account access are separate. CI results still require a GitHub account authorized for this private repository. No remote CI result is claimed. RustSec was obtained through its official GitHub source-archive endpoint when its Git transport failed.

## Remaining owner actions, in order

1. Git transport is working and `release/preflight` is pushed. Authenticate GitHub CLI with an account authorized for `phoenys/slumber` (or grant the existing account access), then confirm `gh repo view phoenys/slumber` works. Review the prepared branch and open a PR. The CI workflow runs on `release/**` branches and PRs; keep the repository private during this stage.

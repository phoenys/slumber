...
job=$(submit "awk 'BEGIN { for (i=0; i<100000; i++) print i }'; awk 'BEGIN { for (i=0; i<100000; i++) print i }' >&2; printf final-out; printf final-err >&2")
wait_done "$job"
[ "$(clean_env "$slumber" logs "$job" | wc -l | tr -d ' ')" -eq 100000 ]
[ "$(clean_env "$slumber" logs "$job" --err | wc -l | tr -d ' ')" -eq 100000 ]
[ "$(clean_env "$slumber" logs "$job" | tail -c 9)" = final-out ]
[ "$(clean_env "$slumber" logs "$job" --err | tail -c 9)" = final-err ]

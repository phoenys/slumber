...
fn init_is_idempotent() {
    ...
    assert!(content.contains("submit the actual foreground command"));
    assert!(content.contains("slumber run --ssh <host>"));
    assert!(content.contains("run-specific completion record"));
    let contract = content.split_once("<!-- SLUMBER PROTOCOL -->").unwrap().1;
    assert!(include_str!("../README.md").contains(contract.trim()));

    let legacy = "# Existing instructions\n\n<!-- SLUMBER PROTOCOL -->\nYou have a background delegator: `slumber run \"<command>\"`.\nRULE:\n- If you need immediate output to write your next command -> run it directly.\n- If your current work is done and you are ready to hand off execution and stop working until it finishes -> use `slumber run \"<command>\"` and exit.\n\n## Project rules\nKeep these rules.\n";
    fs::write(&instructions, legacy).unwrap();
    for _ in 0..2 {
        let output = Command::new(binary)
            .args(["init", "--file"])
            .arg(&instructions)
    ...
    let customized =
        "# Existing instructions\n<!-- SLUMBER PROTOCOL -->\nMy custom handoff rules.\n";
    fs::write(&instructions, customized).unwrap();
    let output = Command::new(binary)
        .args(["init", "--file"])
        .arg(&instructions)
        .output()
    ...
    assert_eq!(fs::read_to_string(&instructions).unwrap(), customized);
...
fn failed_wakeup_is_visible_retryable_and_blocks_stop_while_active() {
    ...
    let output = run.env("REQUEST_SECRET", "snapshot-secret")
        .args(["run", "--resume-template", "test -f \"$SLUMBER_HOME/allow-resume\" || { echo retry-needed >&2; exit 12; }; printf '%s' \"$REQUEST_SECRET\" > \"$SLUMBER_HOME/retried\"", "attempt=0; while [ ! -f \"$SLUMBER_HOME/allow-task\" ] && [ \"$attempt\" -lt 200 ]; do sleep 0.05; attempt=$((attempt + 1)); done"]).output().unwrap();
    assert!(
        output.status.success(),
        "{}",
        String::from_utf8_lossy(&output.stderr)
    ...
    fs::write(root.join("state/allow-task"), "").unwrap();
    for _ in 0..500 {
    ...
    assert!(meta.contains("wake-up command failed"), "{meta}");
...
"#,
    ...
    fs::write(
        ...
        r#"#!/bin/sh
case "$1" in
  display-message) printf '20 40 %%2\n' ;;
  split-window)
    printf '%s\n' "$*" > "$TMUX_TEST_DIR/split"
    printf '%%9\n'
    ;;
...
case "$1" in
  display-message)
    if [ "${FAIL_TMUX-}" = 1 ]; then echo 'pane unavailable' >&2; exit 1; fi
    printf '80 24 %%1\n' ;;
  split-window)
...
"#,
    ...
    fs::remove_file(root.join("state/config.toml")).unwrap();
    let mut failed_pane = Command::new(binary);
    configure(&mut failed_pane);
    let output = failed_pane
        .env("FAIL_TMUX", "1")
        .args(["run", "--no-resume", "true"])
        .output()
        .unwrap();
    assert!(
        output.status.success(),
        "pane failure must not reject a submitted task"
    );
    let warning = String::from_utf8(output.stderr).unwrap();
    assert!(warning.contains("pane unavailable"));
    assert!(warning.contains("Task submission succeeded"));
    thread::sleep(Duration::from_millis(50));
...
#[test]
fn piped_installer_downloads_and_explains_unpublished_assets() {
    let binary = env!("CARGO_BIN_EXE_slumber");
...
case "$url" in
  "https://github.com/phoenys/slumber/releases/$EXPECTED_RELEASE_PATH/slumber-"*) ;;
  *) echo 'unexpected download URL' >&2; exit 1 ;;
esac
if [ "${MISSING_ASSET-}" = 1 ]; then printf 404; exit 22; fi
cp "$TEST_BINARY" "$destination"
printf 200
"#,
    )
    .unwrap();
    ...
    let configure = |command: &mut Command| {
        ...
        command
            ...
            .env("SLUMBER_INSTALL_DIR", root.join("installed"))
            .env_remove("SLUMBER_VERSION")
            .stdin(fs::File::open(&installer).unwrap());
    };
    for (version, path) in [
        (None, "latest/download"),
        (Some("v0.1.0-rc.1"), "download/v0.1.0-rc.1"),
    ] {
        let mut install = Command::new("sh");
        configure(&mut install);
        install
            .env("EXPECTED_RELEASE_PATH", path)
            .arg("-s")
            .arg("--");
    ...
    let mut missing = Command::new("sh");
    configure(&mut missing);
    let output = missing
        .env("EXPECTED_RELEASE_PATH", "latest/download")
        .env("MISSING_ASSET", "1")
        .output()
        .unwrap();
    assert!(!output.status.success());
